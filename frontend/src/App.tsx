  ...
  const fetchAllData = useCallback(async () => {
    try {
      const [
        ...
        axios.get("/api/prs?page=1&limit=50"),
        axios.get("/api/analytics/overview"),
        axios.get("/api/analytics/risk-distribution"),
        axios.get(`/api/analytics/risk-trend?days=${trendPeriod}`),
        axios.get("/api/analytics/repos"),
        axios.get("/api/analytics/intelligence"),
      ]);

      if (Array.isArray(prsRes.data.data)) {
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-emerald-500/30 font-sans">
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-24">
        ...
        {repoStats.length > 0 && (
          <RepoIntelligence data={repoStats} />
        )}
